...
void USBDCore_MainRoutine(USBDCore_TypeDef *pCore)
{
    ...
    /*--------------------------------------------------------------------------------------------------------*/
    if ((pCore->Class.CallBack_MainRoutine.func != NULL) && (pCore->Info.CurrentStatus == USER_USB_STATE_CONFIGURED))
    {
...
{
    return ((pCore->Info.CurrentStatus == USER_USB_STATE_SUSPENDED) ? TRUE : FALSE);
}
...
static void _USBDCore_Standard_SetAddress(USBDCore_TypeDef *pCore)
{
    ...
    pCore->Device.Transfer.Action = USB_ACTION_DATAIN;
    pCore->Info.CurrentStatus = USER_USB_STATE_ADDRESS;
...
static void _USBDCore_Standard_SetConfiguration(USBDCore_TypeDef *pCore)
{
    ...
    pCore->Info.CurrentStatus = USER_USB_STATE_CONFIGURED;
